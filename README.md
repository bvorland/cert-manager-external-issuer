...
│                              Kubernetes Cluster                                  │
│                                                                                  │
│   1. User creates          2. cert-manager           3. External Issuer         │
│      Certificate              creates                   watches for             │
│      resource                 CertificateRequest        CertificateRequests     │
│                                                                                  │
│   ┌──────────────┐        ┌───────────────────┐      ┌────────────────────┐     │
│   │  Certificate │───────▶│ CertificateRequest│─────▶│  External Issuer   │     │
│   │    (CR)      │        │      (CR)         │      │    Controller      │     │
│   └──────────────┘        └───────────────────┘      └─────────┬──────────┘     │
│                                                                 │               │
│                                                                 │ 4. Controller │
...
│                                                      ┌────────────────────┐     │
│                                                      │   ConfigMap with   │     │
│   6. Secret created        5. Signed cert           │   PKI API config   │     │
│      with TLS cert            returned              └─────────┬──────────┘     │
│                                                                 │               │
│   ┌──────────────┐        ┌───────────────────┐                │               │
│   │    Secret    │◀───────│  CertificateRequest│                ▼               │
│   │  (tls.crt,   │        │  Status: Issued   │      ┌────────────────────┐     │
│   │   tls.key)   │        └───────────────────┘      │  External PKI API  │     │
│   └──────────────┘                                   │  (Your CA System)  │     │
│                                                      └────────────────────┘     │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
│  │                    Controller Manager                        │ │
│  │  ┌─────────────────┐ ┌─────────────────┐ ┌───────────────┐  │ │
│  │  │ CertificateReq  │ │     Issuer      │ │ ClusterIssuer │  │ │
